...
function editarCliente(aData){
        ...
        $.ajax({
            ...
            success: function (data) {
              ...
              $("#xCedula").val(formatearNumero(data.respuesta.clientesCedula));
              $("#xNombre").val(data.respuesta.clientesNombres);
              $("#xApellido").val(data.respuesta.clientesApellidos);
              $("#xFecha").val(data.respuesta.clientesFechaNac);
              $("#cCargo").val(data.respuesta.clientesCargo);
              $("#cSexo").val(data.respuesta.clientesSexo);
              $("#xid").val(data.respuesta.idclientes);
        ...
        $('#modal_busqueda_clientes').on('shown.bs.modal', function (e) {
            ...
            sDefaultContent ="<button title='Seleccionar' style='padding:3px' class='btnEditar  btn btn-warning'>" +
                ...
                "</button>";
                /*+
                "<button title='Borrar' style='padding:3px' class='btnReset btn btn-info '>" +
                "<span class='glyphicon glyphicon-refresh'></span>" +
                "</button>"*/
            ;
            cargarTablas("obtenerClientes", "", "#tabla_busqueda_clientes", cambiarDiseno, [0,0],"../Controllers/Modulo1.controller.php",null,sDefaultContent,"#divModal");
        ...
        success: function (data) {
          ...
          $(respuesta).each(function( index,val ) {
            ...
            $('#cCargo').append($('<option>', {
                value: val.idCargos,
                text: val.CargosNombre
            }));
          });
          //  console.log( JSON.parse(data.respuesta))
